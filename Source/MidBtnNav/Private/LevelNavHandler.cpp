...
FLevelNavHandler::FLevelNavHandler() : bIsOrbiting(false), bIsPanning(false), bIsZooming(false), 
                                       LockedDistance(0), CurrentOrbitYaw(0), CurrentOrbitPitch(0),
                                       ViewportClient(nullptr)
{
}

bool FLevelNavHandler::HandleMouseButtonDownEvent(FSlateApplication& SlateApp, const FPointerEvent& MouseEvent, TSharedPtr<SWidget> TargetWidget)
{
	TSharedPtr<SLevelViewport> LevelViewport = StaticCastSharedPtr<SLevelViewport>(TargetWidget);
	if (!LevelViewport.IsValid()) return false;

	ViewportClient = LevelViewport->GetViewportClient().Get();
...
bool FLevelNavHandler::HandleMouseButtonUpEvent(FSlateApplication& SlateApp, const FPointerEvent& MouseEvent)
{
...
bool FLevelNavHandler::HandleMouseMoveEvent(FSlateApplication& SlateApp, const FPointerEvent& MouseEvent)
{
	if (!ViewportClient || (!bIsOrbiting && !bIsPanning && !bIsZooming)) return false;
...
void FLevelNavHandler::ResetState()
{
	bIsOrbiting = bIsPanning = bIsZooming = false;
	ViewportClient = nullptr;
}
